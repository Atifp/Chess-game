...
public class Board {
	...
	//This method requires your input	
	public static Boolean movePiece(int iOld, int jOld, int iNew, int jNew, Piece p){
		if (p.isLegitMove(iOld,jOld,iNew,jNew) ==true){
			if(Board.hasPiece(iNew,jNew) &&( Board.getPiece(iNew,jNew).getSymbol().equals("♚") || Board.getPiece(iNew,jNew).getSymbol().equals("♔"))){
				board[iNew][jNew].removePiece();
				Board.setPiece(iNew,jNew,Board.getPiece(iOld,jOld));
				board[iOld][jOld].removePiece();
				return true;
			}
			if(Board.hasPiece(iNew,jNew) == false){
				Board.setPiece(iNew,jNew,Board.getPiece(iOld,jOld));
				board[iOld][jOld].removePiece();
				return false;
